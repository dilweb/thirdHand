"""Sanitize and compose browser task goals: stable user objective vs operational LLM bundle."""
...
def strip_continuation_slab(text: str) -> str:
    """Remove appended 'Continue the same browser task…' blocks and everything after them."""
    if not text:
        return ""
    raw = text
...
def derive_canonical_objective_from_pending(pending: dict) -> str:
    """Pick a stable high-level objective from Redis pending JSON (supports legacy blobs)."""
    canon = str(pending.get("canonical_user_objective") or "").strip()
    if canon:
        return canon
    for key in ("original_user_request", "user_goal"):
        v = str(pending.get(key) or "").strip()
        if v and "Continue the same browser task" not in v:
            return v
    bg = str(pending.get("browser_goal") or "").strip()
    return strip_continuation_slab(bg)


def truncate_display_title(text: str, *, max_chars: int = 240) -> str:
    """Short single-line / truncated title for Telegram and logs."""
    if not text:
        return ""
...
def build_operational_browser_goal(
    *,
    ...
    """Structured instruction bundle for the browser LLM (no history concatenation)."""
    url_part = (
